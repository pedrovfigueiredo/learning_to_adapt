from learning_to_adapt.dynamics.meta_mlp_dynamics_torch import MetaMLPDynamicsModel
from learning_to_adapt.trainers.mb_trainer import Trainer
from learning_to_adapt.policies.Mmpc_controller import MPCController
from learning_to_adapt.samplers.Msampler import Sampler
from learning_to_adapt.envs.Mnormalized_env import normalize
from learning_to_adapt.utils.utils import ClassEncoder
from learning_to_adapt.samplers.Mmodel_sample_processor import ModelSampleProcessor
...
def run_experiment(config):
    exp_dir = os.path.join(os.getcwd(), 'dump', EXP_NAME, config.get('exp_name', ''))
    if not os.path.exists(exp_dir):
        os.makedirs(exp_dir)
    # exp_dir = os.getcwd() + '/dump/' + EXP_NAME + '/' + config.get('exp_name', '')
    # logger.configure(dir=exp_dir, format_strs=['stdout', 'log', 'csv'], snapshot_mode='last')
    json.dump(config, open(os.path.join(exp_dir, 'params.json'), 'w'), indent=2, sort_keys=True, cls=ClassEncoder)

    env = normalize(config['env'](reset_every_episode=True, task=config['task']))

    # used just for dimensions
