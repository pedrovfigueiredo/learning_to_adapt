    ...
    def fit(self, obs, act, obs_next, epochs=1000, compute_normalization=True,
        ...
        for epoch in range(epochs):
            ...
            for _ in range(num_steps_per_epoch):
                ...
                nn_input_per_task = torch.split(nn_input, self.meta_batch_size, axis=0)
                delta_per_task = torch.split(delta_batch, self.meta_batch_size, axis=0)

                pre_input_per_task, post_input_per_task = zip(*[torch.split(nn_input, 2, axis=0) for nn_input in nn_input_per_task])
                pre_delta_per_task, post_delta_per_task = zip(*[torch.split(delta, 2, axis=0) for delta in delta_per_task])
